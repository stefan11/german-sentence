...
with phenomena that are subsumed under the term of \emph{freezing}\is{freezing} in Mainstream
...
illustrates the phenomenon with reference to the example in (\mex{1}):
\ea[*]{
\gll Was$_1$ denkst du  [\sub{VP$_2$} t$_1$ gelesen] hat keiner t$_2$?\\
     what    think  you {}           {}    read    has no.one\\
\glt Intended: `What do you think nobody has read?'
}
\z
A verb phrase like \emph{das Buch gelesen} `the book read' can be moved resulting in (\mex{1}):
\ea
\gll Das Buch gelesen hat keiner.\\
     the book read    has no.one\\
\glt `Nobody read the book.'
\z
But extraction of the object out of the fronted phrase is impossible as (\mex{-1}) shows. I have
...
\eal
\ex {}[Über dieses Thema]$_i$ [\sub{S} hatte Fritz Peter \_$_j$ gebeten], [[einen Vortrag \_$_i$]
	zu halten]$_j$.\footnote{
        \citep*[\page 21]{HN89b}\iafdata{Hinrichs}\iafdata{Nakazawa}
}
...
finite verb). These approaches predict that subjects are extraction islands \citep[\page 12]{Corver2017a-u}, which is a wrong
prediction. \citet[\page 1094]{HPP2002a-u} provide the following example:
\ea
...
\citep[\page 1094]{HPP2002a-u}
}
\z
\citet{Chaves2013a-u} provides further examples of extractions out of subjects. 


G.\,\citet{GMueller2018a} discusses the examples in (\mex{1}). The first example is probably an
...
difference between these two examples is that (\mex{1}c) contains the participle \emph{gratuliert}
`congratulated' in the prefield while (\mex{1}b) just has a verb trace there.

\eal
\ex[]{
\gll [\sub{CP} [\sub{DP$_1$} Dem        Team] [\sub{PP$_2$} zum    Erfolg] [\sub{C$'$} gratulierte Bernard Hinault]]\\
     {}        {}            the.\dat{} team  {}           to.the success  {}          congratulated Bernard Hinault.\nom\\
}
\ex[*]{
\gll [\sub{CP} Da$_3$ [\sub{DP$_1$} dem         Team] [\sub{PP$_2$} t$_3$ zu] [\sub{C$'$} gratulierte Bernard Hinault]]\\ 
     {}        there  {}      the.\dat{} team   {}           {}    to  {}          congratulated Bernard Hinault.\nom\\
}
\ex[]{
\gll [\sub{CP} Da$_3$ [\sub{DP$_1$} dem         Team] [\sub{PP$_2$} t$_3$ zu] gratuliert    [\sub{C$'$} hat Bernard Hinault]]\\ 
     {}        there  {}      the.\dat{} team   {}           {}    to        congratulated {}  has Bernard Hinault.\nom\\
}
\zl
G.\,Müller wants to explain the ungrammaticality of (\mex{0}b) as a freezing effect, but if \emph{da dem
  Team zu} is one phrase with an empty head one would expect that it behaves like (\mex{0}c) since
...
as in (\mex{0}b--c) does occur in German but it is special. Multiple fronting does occur in German
...
conclusions we draw from such examples. As for (\mex{0}c) I do not share Müller's judgment that
(\mex{0}c) is completely unmarked. If Müller wants to challenge existing analyses it would be his
...
not hold up to scrutiny, freezing cannot be an explanation of the differences in (\mex{0}). Of
course one could state that movement is ruled out in the configurations in (\mex{0}) but this would
be mere stipulation.
...
No difference beteween the conditions with a context
...
denn marks the boundary between topic and focus

> Le 27 août 2019 à 20:02, Stefan Müller <St.Mueller@hu-berlin.de> a écrit :
>
> Great!
>
> By the way, does anybody of you know counter examples to freezing? That
> is scrambling and then extraction out of the scrambled constituent?
>
> Best
>
>     Stefan
...
Also from Chaves 2013 Journal of ling an expectation based account of subject islands and parasitism

Syntactic accounts like the ones above predict that extraction from an extracted phrase is banned in general, as a FREEZING effect (Wexler & Culicover 1980). A conclusion that may be excessive, as the acceptable FREEZING violations in (6) suggest.

(6) (a) ThisisthehandoutithatIcan’tremember[howmanycopiesof__i]j we have to print __ j.

(b) [How many videos __ i] j are there __ j on the web [of Mitt Romney gettingbooed]i ? 



Torr Email:

Ernst 2001 rightward movement
